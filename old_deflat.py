...
def symbolic_execution(start_addr, hook_addr=None, modify=None, inspect=False):
    ...
    if modify != None:
        modify_value = modify
    state = b.factory.blank_state(addr=start_addr, remove_options={LAZY_SOLVES})
    if inspect:
        state.inspect.b('statement', when=BP_BEFORE, action=statement_inspect)
    p = state
...
def get_bytes(data):
    ...
    for item in data:
        if isinstance(item, int):
            res_bytes += bytes(item)
        else:
            res_bytes += item.encode('utf-8')
    return res_bytes
...
if __name__ == '__main__':
    ...
    recovery.write(get_bytes(origin_data))
    recovery.close()
